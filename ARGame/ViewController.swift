...
//  ARGame
//
//  Created by Shashwat Singh on 4/11/22.
//
    ...
    override func viewDidLoad() {
        ...
        cancellable = ModelEntity.loadModelAsync(named: "01")
            ...
            .sink(receiveCompletion: {error in print("Error : \(error)")
                cancellable?.cancel()} , receiveValue:{
            ...
            /*.sink(receiveCompletion: , receiveValue: { error in
            print("Error : \(error)")
            cancellable?.cancel()
        }, recieveValue:{entities in
            var objects : [ModelEntity] = []
            for entity in entities{
                entity.setScale(<SIMD3<Float>(0.002,0.002,0.002),relativeTo : anchor)
                enitity.generateCollisionShapes(recursive : true)
                for _ in 1...2
            }
        })*/
